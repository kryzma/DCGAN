...
class Discriminator(nn.Module):
	def __init__(self, channels_img, features=64):
		super(Discriminator, self).__init__()
		self.net = nn.Sequential(
			nn.Conv2d(
				channels_img, features, kernel_size=4, stride=2, padding=1
			),
			nn.LeakyReLU(0.2),
...
class Generator(nn.Module):
	def __init__(self, z_dim, channels_img, features=64):
		...
		self.net = nn.Sequential(
			...
			nn.ConvTranspose2d(
				features*2, channels_img, kernel_size=4, stride=2, padding=1
			),
			nn.Tanh() 
		)
